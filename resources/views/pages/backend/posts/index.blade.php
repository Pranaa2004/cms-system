    ...
    <div class="container-fluid py-4">
        <div class="row">
            <div class="col-12 align-self-center d-flex justify-content-between align-items-center mb-4">
                ...
                <a href="{{ route('posts.create') }}" class="btn btn-primary shadow-sm px-4 rounded-3">
        ...
        <div class="card shadow-sm border-0 rounded-3">
            <div class="card-body p-4">
                <div class="table-responsive">
                    <table class="table table-hover align-middle w-100" id="posts-table">
    ...
    <script>
        $(document).ready(function() {
            var table = $('#posts-table').DataTable({
                processing: true,
                serverSide: false, // Set to true if you use Yajra server-side
                ajax: {
                ...
                columns: [
                    ...
                    { 
                        data: 'categories',
                        render: function(data, type, row) {
                            if (!data || data.length === 0) return '<span class="text-muted small">None</span>';
                            return data.map(cat => `<span class="badge bg-primary-subtle text-primary border-0 small mb-1 me-1">${cat.name}</span>`).join('');
                        }
                    },
                    { 
                        data: 'tags',
                        render: function(data, type, row) {
                            if (!data || data.length === 0) return '<span class="text-muted small">None</span>';
                            return data.map(tag => `<span class="badge bg-secondary-subtle text-secondary border-0 small mb-1 me-1">${tag.name}</span>`).join('');
                        }
                    },
                    { 
                        data: 'status',
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return `
                                <div class="btn-group">
                                    <a href="/posts/${data}/edit" class="btn btn-sm btn-light text-warning me-1" title="Edit">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    <form action="/posts/${data}" method="POST" class="d-inline-block delete-form">
                                        @csrf
                                        @method('DELETE')
